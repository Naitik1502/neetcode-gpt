...
class Solution:
    def generate(self, model, new_chars: int, context: TensorType[int], context_length: int, int_to_char: dict) -> str:
        ...
        for i in range(new_chars):
            ...
            last_token_logits_probs = nn.functional.softmax(last_token_logits, dim=-1)

            next_token = torch.multinomial(last_token_logits_probs,1, generator=generator)

            context = torch.concat([context, next_token], dim = -1)

            res.append(int_to_char[next_token[0].item()])
