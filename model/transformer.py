...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, model_dim: int, num_heads: int):
        ...
        self.multi_attn = self.MultiHeadedSelfAttention(model_dim, num_heads)
        self.nn = self.VanillaNeuralNetwork(model_dim)
        self.l1 = nn.LayerNorm(model_dim, bias = False)
        self.l2 = nn.LayerNorm(model_dim, bias = False)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        out1 = res + self.multi_attn(self.l1(embedded))
        res  = out1 

        return res + self.nn(self.l2(out1))
